...
// Returns the ASTNode which allocates an array of size num and
// type type.
ASTNode getAllocSequence(Type &type, const std::string &name,
                         AcornSettings &settings,
                         const std::string &num = "1");
...
void downloadPackage(const std::string &url,
                     AcornSettings &settings,
                     const bool &reinstall = false,
...
std::list<std::string> getMacroArgs(
    std::list<Token> &lexed, std::list<Token>::iterator &i);
...
void doRuleAcorn(std::list<Token> &From,
...
void debugPrint(const ASTNode &what, int spaces = 0,
...
// dereferencing.
bool typesAreSameExact(const Type *const a,
                       const Type *const b);
...
candidate.
*/
bool typesAreSameCast(const Type *const a, const Type *const b,
                      int &changes);
