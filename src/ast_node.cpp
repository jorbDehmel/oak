...
void reconstruct(const ASTNode &_what, std::ostream &_where) {
  try {
    if (_what.text == "if") {
      ...
      assert(upon.text == "object");
      Type upon_type = upon.children.front();
      uint upon_derefs = 0;
      while (upon_type.is_ptr()) {
        ++upon_derefs;
        upon_type = upon_type.deref();
      }
      ...
      throw std::runtime_error(
          "Failed to reconstruct unknown node type '" +
          _what.text + "'");
    }
